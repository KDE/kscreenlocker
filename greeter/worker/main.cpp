...
int main(int argc, char *argv[])
{
    ...
    // Safety net to make sure we aren't lingering if our parent process dies.
    // Note that this is threaded because the app thread will eventually block on pam_authenticate.
    std::unique_ptr<QThread> keepAliveThread = [connection /* copy into thread */] {
        auto thread = std::make_unique<QThread>();
        auto keepAliveTimer = new QTimer(thread.get());
        keepAliveTimer->setInterval(2s);
        keepAliveTimer->setSingleShot(false);
        QObject::connect(keepAliveTimer, &QTimer::timeout, keepAliveTimer, [connection] {
            if (!connection.isConnected()) {
                qCDebug(WORKER) << "Parent process has died, quitting worker";
                _exit(0); // don't call exit handlers, we are in a weird zombie state
            }
        });
    ...
    // Make sure to join our threads lest they print warnings about not being joined
    keepAliveThread->quit();
