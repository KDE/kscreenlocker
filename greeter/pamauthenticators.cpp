...
#warning is this still relevant with multiprompt
...
void PamAuthenticators::startAuthenticating()
{
    ...
    if (PAMAuthenticatorModel::instance()->isFunctional(m_authenticator) && d->m_activeAuthenticator) {
        qCDebug(KSCREENLOCKER_GREET) << "PamAuthenticators: Starting interactive authenticator" << d->m_activeAuthenticator->service();
        d->m_activeAuthenticator->tryUnlock();
    }
    if (PAMAuthenticatorModel::instance()->isFunctional(PamAuthenticators::Authenticator::Fingerprint) && d->m_fingerprintAuthenticator) {
        qCDebug(KSCREENLOCKER_GREET) << "PamAuthenticators: Starting non-interactive authenticator" << d->m_fingerprintAuthenticator->service();
...
void PamAuthenticators::onAuthenticatorChanged()
{
    ...
    if (m_authenticator == Authenticator::Fingerprint && d->m_fingerprintAuthenticator) {
        ...
        connect(d->m_fingerprintAuthenticator.get(), &PamAuthenticator::availableChanged, this, [this] {
            ...
            if (!d->m_fingerprintAuthenticator->isReallyAvailable()) {
                PAMAuthenticatorModel::instance()->defunct(PamAuthenticators::Authenticator::Fingerprint);
            }
    ...
    connect(authenticator, &PamAuthenticator::availableChanged, this, [this, authenticator] {
        ...
        d->recomputeNoninteractiveAuthenticationTypes();
        if (!authenticator->isReallyAvailable()) {
            PAMAuthenticatorModel::instance()->defunct(m_authenticator);
        }
        Q_EMIT authenticatorTypesChanged();
    });
