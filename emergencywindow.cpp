...
EmergencyWindow::EmergencyWindow()
{
    ...
    for (auto s : screens) {
        connect(s, &QScreen::geometryChanged, this, &EmergencyWindow::updateGeometry);
    }
    connect(qApp, &QGuiApplication::screenAdded, this, [this](QScreen *s) {
        connect(s, &QScreen::geometryChanged, this, &EmergencyWindow::updateGeometry);
        updateGeometry();
    });
    connect(qApp, &QGuiApplication::screenRemoved, this, &EmergencyWindow::updateGeometry);
}

EmergencyWindow::~EmergencyWindow() = default;
...
void EmergencyWindow::updateGeometry()
{
    QRect combined;
    const auto screens = qApp->screens();
    for (auto s : screens) {
...
#include "moc_emergencywindow.cpp"
